...
现有的去 AI 味工具（[humanizer](https://github.com/blader/humanizer)、[stop-slop](https://github.com/hardikpandya/stop-slop)）都只覆盖英文。本项目填补中文空白，并在以下方面做了改进：
...
```bash
# 方式 1：复制到项目
cp -r SKILL.md references/ your-project/.claude/skills/stop-slop-zh/

# 方式 2：复制到全局
cp -r SKILL.md references/ ~/.claude/skills/stop-slop-zh/
```
...
将 `SKILL.md` 内容复制到 `.cursor/rules/stop-slop.md` 或项目 rules 文件中。
